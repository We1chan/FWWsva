...
offline_recovery="$repo_dir/deploy/tests/gb28181-offline-recovery-smoke.sh"
...
grep -q 'EASYSVA_BACKEND_REF:-6390beca5ee6c08ac8f339872bcb907cca96f635' "$repo_dir/install_source.sh"
grep -q 'EASYSVA_WEB_REF:-98607abc3f598ba5e41a8511d184e1f2899d79e4' "$repo_dir/install_source.sh"
...
grep -q 'sva-backend-gb-ptz.patch' "$repo_dir/install_source.sh"
grep -q 'sva-web-gb-ptz.patch' "$repo_dir/install_source.sh"
grep -q 'Gb28181PtzServiceImpl' "$repo_dir/deploy/patches/sva-backend-gb-ptz.patch"
grep -q 'controlDevicePtz' "$repo_dir/deploy/patches/sva-web-gb-ptz.patch"
grep -q '_decode_ptz_command' "$repo_dir/deploy/patches/sbgb28181-fixed-local-port.patch"
...
grep -q 'GB28181_HEARTBEAT_INTERVAL=10' "$repo_dir/deploy/systemd/easysva-gb-simulator-test6.service"
grep -q 'GB28181_HEARTBEAT_INTERVAL=10' "$repo_dir/deploy/systemd/easysva-gb-simulator-test3.service"
...
grep -q -- '--heartbeat-interval "$heartbeat_interval"' "$simulator_launcher"
grep -q -- '--heartbeat-count "$heartbeat_count"' "$simulator_launcher"
grep -q 'trap restore_camera EXIT INT TERM' "$offline_recovery"
grep -q 'api/device/config/query/basicParam' "$offline_recovery"
